...
export const initializeSupabase = async (): Promise<void> => {
  ...
  try {
    ...
    if (!session) {
      console.log('Auth session not found yet. Skipping database test until user signs in.')
      return
    }
  } catch (e) {
    console.log('Unable to determine auth session, skipping database test.')
    return
  }

  // Only test database after authenticated
  const databaseSuccess = await testDatabaseAccess()
  if (!databaseSuccess) {
    console.warn('⚠️  Database access failed or tables may not be set up yet')
  }
}
